...
- `feat:` localize the 9 protection category titles (`protection_categories.<CATEGORY_ID>`), used by both `ProtectionMenuForm`'s category buttons and `ProtectionCategoryForm`'s form title / success message.
- Multi-language support is now complete across every player-facing form and the protection flag list.
